#!/usr/bin/env python3
"""
Compute ablation-specific similarity matrices using attention-based embeddings.
...
def main():
    ...
    for ablation_name in ['ablation_gru_add', 'ablation_gru_concat', 'ablation_remove_features']:
        ...
        try:
            cmp_embeddings = np.load(f"data/processed_data/cmp_embeddings_{ablation_name}.npz")
            query_FP_list = cmp_embeddings['embeddings']
            
            reference_embeddings = np.load(f"data/processed_data/reference_embeddings_{ablation_name}.npz")
        ...
        np.save(f'data/similarity_metrics/sim_mat_{ablation_name}.npy', sim_mat)
        print(f"Similarity matrix saved to: data/similarity_metrics/sim_mat_{ablation_name}.npy")
