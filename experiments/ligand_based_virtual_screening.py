#!/usr/bin/env python3
"""
Ligand-based virtual screening utilities built on precomputed similarity matrices.
...
def main():
    ...
    for i in range(sim_mat.shape[0]):
        ...
        index_list = np.argsort(tmp_mat)[-1-501:]
        ...
        for idx in index_list:
            if (sim_mat[i][idx] >= 0.65) & (sim_mat[i][idx] < 1):
    ...
    # Save results
    candidate_library_df.to_csv('./data/processed_data/candidate_library.csv', index=False)
    print("Results saved to: ./data/processed_data/candidate_library.csv")
