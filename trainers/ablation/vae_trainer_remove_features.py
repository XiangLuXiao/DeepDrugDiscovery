"""
Ablation training framework for the fingerprint-only molecular VAE variant.

Includes beta scheduling, logging, checkpointing, and GPU-accelerated loops.
...
from models.molecular_vae import MolecularVAE, vae_loss_function
from datasets.ablation.molecularNN_dataset_ablation_remove_features import prepare_vae_dataset
...
class VAETrainer:
    ...
    def prepare_data(self, smiles_list):
        """
        Prepare molecular fingerprint data for training (no features).
        ...
        """
        self.logger.info("Preparing fingerprint-only dataset...")
        
        # Use the dataset module function for processing
        train_loader, val_loader, _ = prepare_vae_dataset(
    ...
    def train_epoch(self, train_loader, epoch):
        ...
        for batch_idx, (fingerprint, feature, target) in enumerate(progress_bar):
            try:
                ...
                # feature is None, so we don't need to move it to device
