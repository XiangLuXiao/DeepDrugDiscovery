"""
Training framework for the molecular VAE including data prep, logging, and scheduling.

Supports beta scheduling, checkpointing, metric visualization, and GPU acceleration.
        ...
        for batch_idx, (fingerprint, feature, target) in enumerate(progress_bar):
            try:
                ...
                if batch_idx == 0 and epoch == 0:  # Only log first batch of first epoch
                    self.logger.info(f"Batch shapes - FP: {fingerprint.shape}, Feat: {feature.shape}, Target: {target.shape}")
                ...
                if self.config['model_params']['feature_size'] > 0:
                    ...
                    reconstruction, mu, logvar = self.model(fingerprint)
