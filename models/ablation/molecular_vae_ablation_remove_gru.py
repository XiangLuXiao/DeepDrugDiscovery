"""
Variant molecular VAE that replaces GRU fusion with lightweight concatenation/addition.

Retains configurable encoder/decoder stacks, beta scheduling, and regularization features.
...
class MolecularVAE(nn.Module):
    """
    A Variational Autoencoder (VAE) for molecular fingerprints with optional features.
    ...
    simple fusion mechanism (concatenation or addition). The model supports variable-depth 
    encoder/decoder stacks and includes modern training stabilization techniques.
    """
    ...
    def __init__(self, args):
        ...
        # Feature fusion method: 'concat', 'add', or 'fingerprint_only'
        self.fusion_method = self.param.get('fusion_method', 'concat')
        ...
        if self.feature_size > 0:
            ...
            # Determine input size for encoder based on fusion method
            if self.fusion_method == 'concat':
                encoder_input_size = self.hidden_size * 2  # Concatenated fingerprint + feature embeddings
            elif self.fusion_method == 'add':
                encoder_input_size = self.hidden_size  # Same size since we add them
            else:
                encoder_input_size = self.hidden_size  # Fingerprint only
        else:
            encoder_input_size = self.hidden_size  # Fingerprint only
    ...
    def _process_inputs(self, fingerprint: torch.Tensor, feature: torch.Tensor = None) -> torch.Tensor:
        """
        Process and optionally fuse fingerprint and auxiliary feature inputs using simple methods.
        
        Encodes the molecular fingerprint through a linear layer and optionally
        fuses it with auxiliary features using concatenation or addition instead of GRU.
        """
        # Encode fingerprint to hidden space
        fp_embed = F.relu(self.fingerprint_encoder(fingerprint))
        
        # Optional feature fusion using simple methods
        if feature is not None and self.feature_size > 0:
            feature_embed = F.relu(self.feature_encoder(feature))
            
            if self.fusion_method == 'concat':
                # Simple concatenation of fingerprint and feature embeddings
                combined_features = torch.cat([fp_embed, feature_embed], dim=-1)
                return combined_features
            elif self.fusion_method == 'add':
                # Element-wise addition of fingerprint and feature embeddings
                combined_features = fp_embed + feature_embed
                return combined_features
            else:
                # Fingerprint only (ignore features)
                return fp_embed
        else:
            return fp_embed
